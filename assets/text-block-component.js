class textBlock extends HTMLElement {
    ...
    handleMouse() {
        if(window.matchMedia("(min-width: 1024px)") && !!this.image) {
            this.querySelector('.text-block-text').addEventListener('mousemove', this.setPosition.bind(this))
            this.querySelector('.text-block-text').addEventListener('mouseenter', this.showImage.bind(this));
            this.querySelector('.text-block-text').addEventListener('mouseleave', this.hideImage.bind(this));
        }
    }
    setPosition(e) {
